...
\begin{frame}
    ...
    \begin{definition}
        ...
        \begin{enumerate}
            ...
            \item If \(A \in \mathcal{F}\) and \(B \subseteq \mathbb{N}\) satisfies \(A \subseteq B\) then \(B \in \mathcal{F}\)
        \end{enumerate}
    \end{definition}
\end{frame}
...
\begin{frame}
    ...
    \begin{definition}
        An ultrafilter on \(\mathbb{N}\) is a maximal filter \(\mathcal{U}\) on \(\mathbb{N}\).
        i.e. if \(\mathcal{F}\) is another filter on \(\mathbb{N}\) such that \(\mathcal{U} \subseteq \mathcal{F}\) then \(\mathcal{U} = \mathcal{F}\).
    \end{definition}
    \pause
    Ultrafilters will be useful in proving the Infinite Ramsey Theorem. 
\end{frame}
...
\begin{frame}
    ...
    \begin{definition}
        Fix \(n \in \mathbb{N}\).
        The filter \(\mathcal{U}_n\) is a principal ultrafilter if it contains all subsets of \(\mathbb{N}\) that include the element \(n\). 
    \end{definition}
    \pause

    \begin{example}
        If a filter \(\mathcal{F}\) contains the singleton set \(\{n\}\) for some \(n \in \mathbb{N}\), then \(\mathcal{F}\) is principal. Moreover, \(\mathcal{F} = \mathcal{U}_n\).
    \end{example}

    We specifically want to use \textit{non-principal} ultrafilters for our results. We will show that such an ultrafilter does in fact exist.
\end{frame}
...
\begin{frame}
    ...
    (i.e. it contains all \(A \subseteq \mathbb{N}\) such that \(\mathbb{N} \setminus A\) is finite.)
    \pause
    \vspace{10pt}
    ...
    \pause
    \vspace{10pt}
    \break Applying this to the Fr\'echet Filter is used to prove the existence of a non-principal ultrafilter. That is, there must be an ultrafilter \(\mathcal{U}\) containing the Fr\'echet Filter.\\
    \vspace{10pt}
    \pause
    Then if \(A \subseteq \mathbb{N}\) is a finite set it cannot be in \(\mathcal{U}\).
    This is since \(\mathbb{N} \setminus A\) is in the Fr\'echet Filter, which is a subset of \(\mathcal{U}\).\\
    \vspace{10pt}
    Therefore \(\mathcal{U}\) cannot be a principal ultrafilter. \qed 
\end{frame}
...
\begin{frame}
    \frametitle{Infinite Ramsey Theorem}
    \begin{theorem}[Infinite Ramsey Theorem]
        Given any 2-colouring \(\gamma: \mathbb{N} \to \{\text{\textcolor{red}{red}}, \text{\textcolor{blue}{blue}}\}\) of the edges of a complete graph on the natural numbers, there exists an infinite monochromatic subgraph.
    \end{theorem}
\end{frame}
...
\begin{frame}
    ...
    \pause
    \vspace{10pt}
    \break Consider the complete graph on the natural numbers, a \(\text{\textcolor{red}{red}}\)-\(\text{\textcolor{blue}{blue}}\) colouring on this graph, and \(\mathcal{U}\), a non-principal ultrafilter on \(\mathbb{N}\).\\
    \vspace{10pt}
    \pause
    Fixing a particular vertex \(v\) we can partition \(\mathbb{N}\) into the sets:
    \pause
    \begin{enumerate}
        \item \(v\) itself, \(\{v\}\)
        \item \(A_v = \) all vertices which share a \(\text{\textcolor{red}{red}}\) edge with \(v\)
        \item \(B_v = \) all vertices which share a \(\text{\textcolor{blue}{blue}}\) edge with \(v\)
    \end{enumerate}
    ...
    \pause
     ...
     Now since \(\mathcal{U}\) is non-principal, the set \(\{v\}\) is not in \(\mathcal{U}\).
     So it must be that exactly one of \(A_v\) or \(B_v\) is in \(\mathcal{U}\). So, suppose \(B_v \in \mathcal{U}\).\\
    \vspace{10pt}
    Then, we can keep picking verticles \(v_i\) from \(B_v\) such that \(B_{v_i} \in \mathcal{U}\) to obtain an infinite set of vertices which all share \textcolor{blue}{blue} edges. Hence, we get a monochromatic infinite complete subgraph. \qed
\end{frame}
...
\begin{frame}
    ...
    \begin{example}
        Given \(A = \{1, 2, 3, 4, 5\}\) and \(n = 2\), we have \(A - n = \{1, 2, 3\}\).
    \end{example}
...
\begin{frame}
    ...
    \begin{block}{Closure}
        If \(\mathcal{U}\) and \(\mathcal{W}\) are ultrafilters on \(\mathbb{N}\), then \(\mathcal{W}+\mathcal{U}\) is also an ultrafilter.
    \end{block}
    ...
    In this example, \(\mathcal{U}_n + \mathcal{U}_m = \mathcal{U}_{n+m} = \mathcal{U}_m + \mathcal{U}_n\). Is this always the case?
...
\begin{frame}
    ...
    \vspace{10pt}
    \pause
    Then if \(\mathcal{U}\) is idempotent with \(A \in \mathcal{U}\):
    \begin{itemize}
...
\begin{frame}
    ...
    Since \(\mathcal{U}\) is idempotent, we have \(A^* = A \cap \{n \in \mathbb{N}: A - n \in \mathcal{U}\} \in \mathcal{U}\). So, pick \(x \in A^*\). We get that: \pause

    \begin{itemize}
        \item \(x\) is \textcolor{red}{red}, since \(x \in A\)
        \item \(A-x \in \mathcal{U}\)
    \end{itemize}
    ...
    Let \(A_1 = A \cap (A-x) \in \mathcal{U}\) and pick \(y \in A_1\). Then, we have:\\
    \begin{itemize}
        \item \(y\) is \textcolor{red}{red}, since \(y \in A\)
        \item \(x + y \in A\), since \(y \in A-x\) \pause
    \end{itemize}
    
    \vspace{10pt}

    Therefore \(x, y\), and \(x + y\) are all \(\gamma\)-\textcolor{red}{red}! \qed
...
\begin{frame}
    ...
    We can carry on a (similar) construction indefinitely to obtain an \textit{infinite} subset.

    \begin{theorem}[Hindman's Theorem]
        Let \(\gamma: \mathbb{N} \to \{\text{\textcolor{red}{red}}, \text{\textcolor{blue}{blue}}\}\) be a 2-colouring of \(\mathbb{N}\). Then there exists an infinite set \(M = \{x_1, x_2, ... \} \subseteq \mathbb{N}\) such that FS(\(M\)) is monochromatic.
    \end{theorem}
...
\begin{frame}
    ...
    What about finite cases?

    \pause

    \vspace{10pt}

    What about more than two colours?
\end{frame}
