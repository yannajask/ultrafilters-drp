...
\begin{frame}
    ...
    Importantly, we get that \(n + \mathcal{U}\) is also an ultrafilter.
\end{frame}

\begin{frame}
    \frametitle{Ultrafilter Addition}

    We can extend this notion to define addition between two ultrafilters:\\[10pt]
...
\begin{frame}
    ...
    \begin{theorem}[Hindman's Theorem]
        Let \(\gamma: \mathbb{N} \to [2]\) be a 2-colouring of \(\mathbb{N}\). Then there exists an infinite subset \(M \subseteq \mathbb{N}\) such that FS(\(M\)) is monochromatic.
    \end{theorem}
